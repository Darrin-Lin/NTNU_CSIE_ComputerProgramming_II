...
static int8_t ytox(int64_t **pVResultPowers, int64_t **pVResultCoefficients, size_t *size, int64_t *pXPowers, int64_t *pXCoefficients, uint32_t xsize, uint32_t power, int32_t ycof, int64_t *pXpPowers, int64_t *pXpCoefficients, uint32_t xpsize);
static int8_t dot(int64_t *pAPowers, int64_t *pACoefficients, size_t aSize, int64_t *pBPowers, int64_t *pBCoefficients, size_t bSize, int64_t **pResultPowers, int64_t **pResultCoefficients, size_t *size);
static int8_t merge(int64_t **pTargetPowers, int64_t **pTargetCoefficients, size_t *targetSize);
...
static int8_t merge(int64_t **pTargetPowers, int64_t **pTargetCoefficients, size_t *targetSize)
{
    if (pTargetPowers == NULL || pTargetCoefficients == NULL)
    {
        return -1;
    }
    // sort the powers
...
static int8_t ytox(int64_t **pVResultPowers, int64_t **pVResultCoefficients, size_t *size, int64_t *pXPowers, int64_t *pXCoefficients, uint32_t xsize, uint32_t power, int32_t ycof, int64_t *pXpPowers, int64_t *pXpCoefficients, uint32_t xpsize)
{
    ...
    for (uint32_t i = 0; i < power; i++)
    {
        ...
        size_t tsize = 0;
        dot((int64_t *)pXPowers, (int64_t *)pXCoefficients, xpsize, vecP, vecC, vsize, &tempP, &tempC, &tsize);
        for (uint32_t j = 0; j < tsize; j++)
        {
            Vector_push_back(&vecP, *(tempP + j), (size_t)vsize);
            Vector_push_back(&vecC, *(tempC + j), (size_t)vsize);
            vsize++;
        }
        Vector_free(tempP);
        Vector_free(tempC);
        merge(&vecP, &vecC, (size_t *)&vsize);
    }
    int64_t *tempP = Vector_create(0);
    int64_t *tempC = Vector_create(0);
    size_t tsize = 0;
...
int32_t chain_rule(sPoly *pResult, const sPoly *pFy, const sPoly *pFx)
{
    ...
    merge(&Vector_yP_powers, &Vector_yP_coefficients,(size_t *) &ypsize);
    for (uint32_t i = 0; i < ypsize; i++)
    {
    ...
    merge(&Vector_xP_powers, &Vector_xP_coefficients,(size_t *) &xpsize);
